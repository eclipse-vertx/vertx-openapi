...
class RequestUtilsTest extends HttpServerTestBase {
  ...
  void testExtractHeader(Parameter parameter, Map<String, String> headers, String expected,
    ...
    createValidationHandler(params -> {
      Truth.assertThat(params.getHeaders().get(parameter.getName()).getString()).isEqualTo(expected);
      testContext.completeNow();
    }, mockOperation(parameter), testContext).compose(v -> createRequest(HttpMethod.GET, "").map(req -> {
  ...
  void testExtractPath(Parameter parameter, Style style, String path, String expected, VertxTestContext testContext) {
    ...
    createValidationHandler(params -> {
      Truth.assertThat(params.getPathParameters().get(parameter.getName()).getString()).isEqualTo(expected);
      testContext.completeNow();
  ...
  void testExtractBody(VertxTestContext testContext) {
    ...
    createValidationHandler(params -> {
      Truth.assertThat(params.getContentType()).isEqualTo(APPLICATION_JSON.toString());
      Truth.assertThat(params.getBody().isBuffer()).isTrue();
      Truth.assertThat(params.getBody().getBuffer().toJsonObject()).isEqualTo(bodyJson);
      testContext.completeNow();
